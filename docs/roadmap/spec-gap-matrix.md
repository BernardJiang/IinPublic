...
| Survey and statistics | FR-SV-1..6, UI-4, §12.7, §13.5 | Implemented baseline | Survey talks, generic stats endpoints (`summary`, `by-day`, `by-region`, `by-answer`), Talks-row Results modal, dedicated survey analytics dashboard, low-count anonymity masking, CSV exports, follow-up survey creation, broadcast tag popularity/trends, peer relationship stats, and UTC boundary coverage are implemented. Coverage includes `tests/e2e/20-survey-analytics-dashboard.spec.ts`, talks-matching survey specs, and `src/test/integration/talk-loop.test.ts` | Future work is richer statistics: cross-question survey analytics, consistent day/week/month UI, chatroom/location analytics, peer/reputation analytics polish, durable stats persistence, and a unified dashboard. |
