...
- Wire encrypted P2P handoff transfer and receiver import (spec §11.2). New
...
- Enable and pass a real send→ack→import round trip:
...
## 2026-09-08 — §I: Multi-device identity linking (landed items archived)

Archived from `docs/TODO.md` Priority 2 §I. Two items stay active in `docs/TODO.md`: password-free custody hardening beyond v1, and X3 (website↔app linking, blocked on a native-shell CI runner).

- Freeze v1 semantics: direct mutual `LINK_IDENTITY` edges between independent SEA identities;
...
- Start the **Identity & devices** Settings shell with identity fingerprint/status, local
  protection status, renameable privacy-minimized installation metadata, and linked identities.
- Review and approve `docs/security/local-identity-password-custody-design.md` for staged
...
- Wire `WebIdentityLinkService` into `app.ts`; entered codes now publish a real SEA-signed
  one-sided attestation and display **Waiting for approval** until mutual confirmation exists.
- Complete the two-installation mutual approval path with a versioned expiring code, real QR,
...
- Make removal local-first with a durable signed revocation outbox, immediate trust denial,
...
- Wire URL-fragment, loopback, and clipboard same-device linking shortcuts (spec §10.3).
...
- Enable and pass X8 same-device linking E2E. **Landed 2026-08-26** — see
...
## 2026-09-08 — §BB: Typed opposite-tag matching follow-ups (fully landed)

Archived from `docs/TODO.md` Priority 4 §BB — all four follow-up items shipped.
...
- Persist user-created opposite-tag pairs; seeded pairs already work. **Landed 2026-08-23** —
  see `docs/completed.md`.
- Support talk-level shared time/location questions before route item branches. **Landed
  2026-08-23** — see `docs/completed.md`.
- Add E2E cases for missing preference and `location`'s unconditional-ASK_USER fallback to
...
## 2026-09-08 — §DD: Generalized dating matching — age/preference-set primitives landed

Archived from `docs/TODO.md` Priority 4 §DD. One item stays active in `docs/TODO.md`: optional author-selected talk photo delivery after a match, plus its safety notice — left for a dedicated product/safety pass.

- Implement mutual preference-set membership and the age point-in-range primitive. **Landed
...
- Wire `ageRange` as a real `BuiltInQuestionKind`, and force+lock `isAdult` for talks that use
  it. **Landed 2026-08-24** — see `docs/completed.md`.
- Wire multi-value gender/race preference matching. **Landed 2026-08-27** — see
...
## 2026-09-08 — §EE: Me/profile completion — spec matrix update landed

Archived from `docs/TODO.md` Priority 4 §EE. Two items stay active in `docs/TODO.md`: storing typed built-in declarations as `AnswerRecord` values, and its dependent E2E coverage — both blocked on a real product decision about where a self-authored builtIn declaration should surface (see the research note retained in `docs/TODO.md`).

- Update the technical-specification implementation matrix. **Landed 2026-08-23** — Appendix
...
## 2026-09-08 — §LL/§LL.1/§LL.2: unify `type: 'tag'` and pair-tag matching (fully landed)

Archived from `docs/TODO.md` Priority 4. All three design passes shipped same-day as designed; nothing remains open under these letters.
...
- `type: 'tag'` stayed a distinct wire type — still renders as a one-line chip, no Gun data
  migration; its answer TEXT (via the new `#talk-answer` field) became meaningful directly, no
  talk-level metadata involved.
- `checkIfMatch`'s veto code was left in place (untouched) but is now a permanent no-op for
...
- No migration/rewrite for existing stored talks — old flow/route talks with no
  ...
  tag talks changed.
- Confirmed the seeded opposite-tag registry (`tag-opposite-pairs.ts`) is editor-autofill only
...
## 2026-09-08 — §K7: Delegated TechSupport answers — design + implementation landed

Archived from `docs/TODO.md` Priority 5 §K7. Two items stay active in `docs/TODO.md`: production TechSupport key custody/rotation tooling, and packaging the headless/off-server TechSupport agent.
...
- Implement only after that design is approved. — approved (delegation-credential model;
...
## 2026-09-08 — UI god-object refactor: Issue #2 (unused React dependency graph) removed

Archived from `docs/TODO.md` Priority 6. Issue #1 (`ui-manager.ts` decomposition) remains active and in progress; this covers only the fully-shipped Issue #2 evidence and removal checklist.
...
- **2.1 Freeze the evidence.** ✅ (2026-08-22) `find src -name '*.tsx'` → `0`; no react/ReactDOM imports outside the examples; no `react` in `.eslintrc`; no `@testing-library/react` usage. Confirmed. Also found `cytoscape-dagre` is dead too (missing from the table) — removed it as well.
...
- **2.2 Decide disposition of `src/examples/`** ✅ owner chose **archive + drop deps**: moved to `docs/archive/examples/` via `git mv` (88 files, git history preserved); note added to `docs/archive/README.md`.
...
- **2.3 Remove the dead `devDependencies`** from `package.json` (2.2 = archive) ✅ dropped: `react`, `react-dom`, `react-cytoscapejs`, `cytoscape`, `@testing-library/react`, `@babel/preset-react`, and **`cytoscape-dagre`** (extra vs table).
      `react`, `react-dom`, `react-cytoscapejs`, `cytoscape`, `@testing-library/react`,
      `@babel/preset-react`.
- **2.4 Remove `@babel/preset-react`** from `babel.config.js` (line 1) ✅ kept `@babel/env` + the transform plugins.
...
- **2.6 Reinstall to sync the lockfile:** ✅ did `npm install` (deviation: lighter than the full `rm -rf` wipe, same lockfile result — npm removed 21 packages: the 7 + transitive react-dom/cytoscape tree). Also regenerated `docs/dependency-sbom.json` via `npm run sbom`.
      (this is a devDependency-only change; do **not** `npm prune --production` before any build).
- **2.7 Verify:** ✅ all green (2026-08-22): `test:type` RC0, `lint` RC0, `test:unit` 1478/1478, `build:web` (bundle.js 1.3M > 500KB), `build:server` OK, live boot HTTPS :8080 serving SPA at `/` + `/health` → 200.
...
- **2.8 Commit** — done as `2f0b7355` on branch `refactor/ui-decompose-and-deps-clean`.
