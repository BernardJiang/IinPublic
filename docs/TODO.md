...
- [x] **Wire `config.gun` at the live construction sites** so Gun-signaling is the default: `peer-mesh-service.ts`, `direct-p2p-conversation-transport.ts`, `app.ts` pass `gun: this.gunService.getGun()`. HTTP `P2PSignalingClient` remains the no-gun fallback.
...
- [ ] **VERIFY (browser CI):** run `npx playwright test tests/e2e/staged/stage2-two-user/` and confirm the 4 messaging/handshake specs above pass with Gun signaling as default.
