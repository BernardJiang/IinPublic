...
- [ ] **Wire `config.gun` at the live construction sites** so Gun-signaling is the default. **Attempted 2026-06-16 (`peer-mesh-service.ts`, `direct-p2p-conversation-transport.ts`, `app.ts`) then REVERTED** — flipping the default red-lined 5 stage2 two-user E2E specs (peers never complete the WebRTC handshake over Gun pub/sub): `00j-messaging-edge-cases` (reopen-after-reload, messaging-after-unblock), `00k-p2p-handshake`, `09-messaging`, `10-message-unread-badge`. Root cause is in `GunPubSubSignaler`'s Gun replication/subscription path (frames written to `p2p-signal/<sharedKey>/<nonce>` are not reaching the peer's `.map().on()` subscription in-browser); needs debugging against a live Gun relay (cannot reproduce in Jest). **Before retrying the default flip**, reproduce with `npx playwright test tests/e2e/staged/stage2-two-user/00k-p2p-handshake.spec.ts` and confirm offer/answer frames replicate peer→peer. Likely suspects: primitive-string leaves under `.map()`, `.map().on()` not replaying children that arrived before subscribe, or the server relay not broadcasting `p2p-signal` puts. HTTP `P2PSignalingClient` remains the default until this is fixed.
