...
- [ ] **VERIFY (browser):** `npx playwright test tests/e2e/staged/stage2-two-user/` — confirm messaging/handshake specs still pass with pure `.on()` push (no poll). NOTE: AXE is disabled in E2E for parallel-hub isolation; if pure push proves unreliable across two browsers without AXE, options are (a) enable AXE for single-worker runs or (b) revisit — but NOT a server relay.
