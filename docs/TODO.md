...
- [~] **T2. UI follow-up for distance sort (spec §22.7).** `[Sonnet]` Core sort logic DONE 2026-06-13 (moved to `docs/completed.md`). **Audit 2026-06-14:** `rankPeople` from `find-similar.ts` is never called by `contacts-view.ts` — the view has its own independent inline sort switch (lines 670–690). The `SORT_STRATEGIES` registry is wired through `ContactsViewDeps` but only populates the UI dropdown; it does not drive sorting. Step **(a) DONE 2026-06-14:** added `distanceMiles?: (userId: string) => number | undefined` to `ContactsViewDeps`, added `distance` branch to the sort switch (ascending, peers without distance sort last), passed `distanceMiles: undefined` at all 3 `ui-manager.ts` call sites. Step **(b) pending:** wire a real distance resolver — requires reading peer GPS from Gun public profiles (not in `PeerSummary` today); file as separate task.
