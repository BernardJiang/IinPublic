...
## 14. Keep the TechSupport root off the VPS

Normal relay startup publishes the committed, pre-signed TechSupport identity and Global presence
floor without any private key. Keep all `TECHSUPPORT_KEY_*` and
`TECHSUPPORT_SEA_PAIR_JSON` variables unset in the VPS environment. Do not install the root vault,
run `npm run techsupport:agent`, or create a TechSupport `systemd`/`tmux` process on this host.

Use short-lived delegate accounts on normal application devices for routine support. The root is
