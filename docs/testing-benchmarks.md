...
### Fix: Isolate Disk Clears Per-Worker

`clearGunDatabases()` already calls `parallelSlot()` for the in-memory clear. The disk clears need the same treatment:

```typescript
// Current: shared paths (BUG)
const radataPath = path.join(__dirname, '../../../radata');

// Fixed: per-worker paths
...
However, this requires server startup to also use per-worker disk paths. Since servers already use `E2E_GUN_MEMORY_ONLY=1`, the disk clears are actually unnecessary — remove them entirely.
...
- [ ] Remove shared disk deletes from `clearGunDatabases()` (keep only in-memory clear via `/api/test/clear-database`)
...
- [ ] Add retry or explicit synchronization for tests that depend on Gun graph stability after clear
- [ ] W1-W2 remain recommended for CI (minimal server spawn overhead, no interference risk)
