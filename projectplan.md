...
- **FR‑UM‑1**: The system SHALL generate a unique **SEA public/private key pair** (using Gun.js SEA) upon first application start. The Public Key SHALL serve as the User ID.
- **FR‑UM‑2**: The system SHALL prompt the user for a **StageName** immediately upon first start, defaulting to "**John Doe**". Users SHALL be able to change this name at any time.
...
- **FR‑CR‑12**: Upon entering a chatroom, the system SHALL provide a single action (or configure as automatic) to **broadcast all active talks** to all users in the room.
- **FR‑CR‑13**: The system SHALL automatically request/subscribe to public talks from all other users in the room upon entry.
- **FR‑CR‑14**: The system SHALL use **last-exchange timestamps** per pair of users to ignore talks that have already been sent/received, preventing duplication.
