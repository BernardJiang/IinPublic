 ...
 */
export const saveUserProfile = async (user: any, profile: UserProfile): Promise<void> => {
    ...
    return new Promise((resolve, reject) => {
        ...
        user.get('profile').put(dataToSave, (ack: any) => {
 ...
 */
export const subscribeToUserProfile = (gun: any, pubKey: string, callback: (profile: UserProfile | null) => void) => {
    gun.user(pubKey).get('profile').on((data: any) => {
        if (!data) {
            callback(null);
            return;
        }
