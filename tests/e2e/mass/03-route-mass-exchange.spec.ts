...
test.describe('M3 real route mass exchange', () => {
  test('eight browsers deliver one route and produce seven distinct numbered leaves', async () => {
    ...
    try {
      for(let i=0;i<8;i+=1){const x=await bootstrapUser(browser,`M3-${i}`,`M3 User ${i}`);contexts.push(x.context);pages.push(x.page);await x.page.locator('.chatroom-item:has-text("Global")').first().click();await x.page.evaluate(()=> (window as any).__iinpublic_app.getApp().setTalkLedgerQuotaUnlimitedForE2e(true));}
    ...
    } finally {await Promise.all(pages.map(p=>p.evaluate(()=> (window as any).__iinpublic_app?.getApp?.()?.manualCleanup?.()).catch(()=>{})));await Promise.all(contexts.map(c=>c.close().catch(()=>{})));await browser.close().catch(()=>{});await maybeClearGunDatabases();}
  });
});
