...
This is the very first step of the staged pipeline (`E2E_STAGE_PIPELINE=1`). It wipes all existing Gun data so every stage starts from a known-empty baseline, creates and logs in the TechSupport bootstrap root, verifies the UI shows "Global" chatroom with headcount 1, then saves the storage state snapshot.
...
3. **Bootstrap TechSupport root:** Logs in via `bootstrapTechSupport()`, joins Global chatroom.
