...
> 1. The "already sent" ledger check that gates catch-up (`isBroadcastUnsentForReceiver`, now in `broadcast-delivery-selection.ts`) compared the whole-talk identity key, but tag-talk delivery records suppression under different, per-tag identity keys (`buildTagIdentityKeys`). A tag talk always read as "unsent" for everyone, masking whether catch-up delivery itself worked.
