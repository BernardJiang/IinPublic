...
verify a settings toggle survives a reload, and exercise HTTP, WebSocket,
localStorage, IndexedDB, and Gun persistence in the active browser engine.
...
Run one desktop engine with `npm run test:e2e:webkit` or
`npm run test:e2e:firefox`. Run the complete desktop-engine smoke matrix with
`npm run test:e2e:browsers`; all three projects are included in the same
