// Test core functionality by importing actual modules
// This tests the actual Phase 1 implementation
...
// Mock Gun for basic functionality
const mockGun = {
  _data: {},
  get: function(path) {
  ...
  put: function(data) {
    this._data[this._currentPath] = { ...this._data[this._currentPath], ...data }
    return Promise.resolve()
  },
  once: function() {
    return Promise.resolve(this._data[this._currentPath] || {})
  },
  on: function(callback) {
    setTimeout(() => callback(this._data[this._currentPath] || {}), 0)
    return this
  },
  user: function() {
    return this
  ...
  set: function(item) {
    this._data[this._currentPath] = { ...this._data[this._currentPath], ...item }
    return this
  }
}
...
describe('Phase 1: Core Infrastructure - Integration Tests', () => {
  ...
  test('ChatroomManager - GPS hash generation', () => {
    // Simulate the ChatroomManager functionality
  ...
  test('TalkManager - Question validation', () => {
    ...
    const invalidQuestions = [
      ...
      "??",
      "Very long question that exceeds the five hundred character limit and should be rejected by the validation system".repeat(10)
    ]
  ...
  test('Auto-capture pattern detection', () => {
    ...
    // Test pattern matching - working regex
    const pattern = /([^?]+)\?(.+);(.+)\./
    ...
    validPatterns.forEach(message => {
      ...
      // matches[1] = question, matches[2] = final answer
      console.log('Pattern matches for:', message, matches)
      expect(matches[1] + '?').toMatch(/\?$/)
      expect(matches[2] + '.').toMatch(/\.$/)
    })
